...
const CONTACT_EMAIL = (process.env.CONTACT_EMAIL || process.env.ADMIN_EMAIL || 'gaahdesigner@gmail.com').toString().trim();
...
route('GET', '/politicadeprivacidade', (req, res) => {
  ...
  const home = 'https://zapmro.com.br';
  const delUrl = `${home}/exclusaodedados`;
  html(res, `<!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Política de Privacidade - ZAPMRO</title><link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg%20xmlns=%27http://www.w3.org/2000/svg%27%20viewBox=%270%200%2064%2064%27%3E%3Ccircle%20cx=%2732%27%20cy=%2732%27%20r=%2730%27%20fill=%27%2325d366%27/%3E%3Cpath%20fill=%27%23ffffff%27%20d=%27M42.5%2036.8c-1.1-.6-2.5-1.3-3.7-1.8-.4-.2-.9-.1-1.2.2l-2%202.4c-.3.3-.7.4-1.1.3-2.7-1.1-5.9-4.2-7-7-.1-.4%200-.8.3-1.1l2.4-2c.3-.3.4-.8.2-1.2-.5-1.2-1.2-2.6-1.8-3.7-.2-.4-.6-.7-1.1-.7H23c-.6%200-1.1.4-1.2%201-1.1%206.3%201%2012.6%205.9%2017.5%204.9%204.9%2011.2%207%2017.5%205.9.6-.1%201-.6%201-1.2v-3.1c0-.5-.3-.9-.7-1.1z%27/%3E%3C/svg%3E"><style>body{font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;max-width:900px;margin:40px auto;padding:0 16px;line-height:1.55;color:#111}h1{font-size:28px;margin:0 0 8px}h2{font-size:18px;margin:26px 0 8px}h3{font-size:16px;margin:20px 0 8px}p,li{color:#333}a{color:#128c7e}small{color:#666}.box{background:#fff;border:1px solid #e0e0e0;border-radius:14px;padding:14px 16px;margin:16px 0}.muted{color:#667781}</style></head><body><h1>Política de Privacidade</h1><small>Última atualização: ${today}</small><div class="box"><p class="muted">Esta Política explica como o ZAPMRO (“nós”) trata dados pessoais quando você usa o site e o painel (CRM/automação) integrados a serviços como WhatsApp e Google.</p><p class="muted">Ao usar o ZAPMRO, você concorda com esta Política. Se não concordar, não utilize o serviço.</p></div><h2>1. Quem controla os dados</h2><p>O ZAPMRO atua como operador/controlador conforme o contexto. Quando você conecta suas contas (ex.: WhatsApp/Google), você pode inserir e gerenciar dados de terceiros (ex.: contatos e conversas) no painel.</p><h2>2. Dados que coletamos</h2><h3>2.1 Dados de conta</h3><ul><li>Nome, e-mail, identificadores de login (ex.: Facebook ID quando você escolhe entrar com Facebook).</li><li>Credenciais: senhas são armazenadas com hash (não guardamos senha em texto puro).</li></ul><h3>2.2 Dados operacionais do painel</h3><ul><li>Sessões, preferências, configurações (ex.: funis/kanban, automações, horários de envio).</li><li>Registros de uso e eventos do sistema (ex.: logs de execução e auditoria administrativa).</li></ul><h3>2.3 Dados de integrações</h3><ul><li>Tokens e escopos quando você autoriza integrações (ex.: Google Contatos).</li><li>Dados necessários para operar a integração (ex.: IDs técnicos do WhatsApp Business/Cloud API e eventos de webhook).</li></ul><h3>2.4 Dados de comunicação (quando aplicável)</h3><ul><li>Mensagens e metadados necessários para exibir e operar atendimentos no CRM, incluindo identificação de conversas/contatos e histórico.</li></ul><h2>3. Finalidades e bases de uso</h2><ul><li><strong>Prestar o serviço:</strong> permitir login, operar o CRM, enviar mensagens conforme sua ação/automação e manter histórico.</li><li><strong>Segurança e prevenção a abuso:</strong> proteger contas, detectar acessos indevidos e manter estabilidade do serviço.</li><li><strong>Melhorias:</strong> corrigir erros e melhorar desempenho, com base em dados técnicos e operacionais.</li></ul><h2>4. Compartilhamento</h2><p>Não vendemos dados. Compartilhamos somente quando necessário:</p><ul><li>Com provedores de infraestrutura (hospedagem, rede, armazenamento) para operar o serviço.</li><li>Com plataformas integradas mediante sua ação/autorização (ex.: Google, Meta/WhatsApp).</li><li>Para cumprir obrigação legal, ordem judicial ou exigência regulatória.</li></ul><h2>5. Transferência internacional</h2><p>Integrações e infraestrutura podem envolver processamento fora do Brasil, conforme políticas dos provedores (ex.: Google/Meta). Buscamos usar configurações e medidas de segurança compatíveis com boas práticas.</p><h2>6. Retenção</h2><p>Retemos dados pelo tempo necessário para operar o serviço, cumprir obrigações e permitir histórico/configurações. Você pode solicitar exclusão conforme indicado em “Exclusão de dados”.</p><h2>7. Direitos do titular</h2><p>Você pode solicitar: confirmação de tratamento, acesso, correção, portabilidade quando aplicável, anonimização/bloqueio, exclusão e informações sobre compartilhamentos.</p><h2>8. Segurança</h2><p>Adotamos medidas técnicas e administrativas para proteger dados contra acesso não autorizado, perda e alteração. Ainda assim, nenhum sistema é 100% seguro.</p><h2>9. Cookies</h2><p>Podemos usar cookies/local storage para manter sessão, preferências e funcionamento do painel. Você pode ajustar seu navegador para bloquear cookies, mas isso pode afetar o uso do serviço.</p><h2>10. Exclusão de dados</h2><p>Para instruções de exclusão, acesse: <a href="${delUrl}">${delUrl}</a>.</p><h2>11. Contato</h2><p>Dúvidas sobre privacidade: <a href="mailto:${escapeHtmlServer(CONTACT_EMAIL)}">${escapeHtmlServer(CONTACT_EMAIL)}</a> ou <a href="${home}">${home}</a>.</p><h2>12. Alterações</h2><p>Podemos atualizar esta Política periodicamente. A data de “última atualização” indica a versão vigente.</p></body></html>`);
});
route('GET', '/politicadeprivacidade/', routes.GET['/politicadeprivacidade']);
route('GET', '/br/politicadeprivacidade', routes.GET['/politicadeprivacidade']);
route('GET', '/br/politicadeprivacidade/', routes.GET['/politicadeprivacidade']);

route('GET', '/termosdoservico', (req, res) => {
  const today = new Date().toISOString().slice(0, 10);
  const home = 'https://zapmro.com.br';
  const pp = `${home}/politicadeprivacidade`;
  html(res, `<!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Termos de Serviço - ZAPMRO</title><link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg%20xmlns=%27http://www.w3.org/2000/svg%27%20viewBox=%270%200%2064%2064%27%3E%3Ccircle%20cx=%2732%27%20cy=%2732%27%20r=%2730%27%20fill=%27%2325d366%27/%3E%3Cpath%20fill=%27%23ffffff%27%20d=%27M42.5%2036.8c-1.1-.6-2.5-1.3-3.7-1.8-.4-.2-.9-.1-1.2.2l-2%202.4c-.3.3-.7.4-1.1.3-2.7-1.1-5.9-4.2-7-7-.1-.4%200-.8.3-1.1l2.4-2c.3-.3.4-.8.2-1.2-.5-1.2-1.2-2.6-1.8-3.7-.2-.4-.6-.7-1.1-.7H23c-.6%200-1.1.4-1.2%201-1.1%206.3%201%2012.6%205.9%2017.5%204.9%204.9%2011.2%207%2017.5%205.9.6-.1%201-.6%201-1.2v-3.1c0-.5-.3-.9-.7-1.1z%27/%3E%3C/svg%3E"><style>body{font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;max-width:900px;margin:40px auto;padding:0 16px;line-height:1.55;color:#111}h1{font-size:28px;margin:0 0 8px}h2{font-size:18px;margin:26px 0 8px}p,li{color:#333}a{color:#128c7e}small{color:#666}.box{background:#fff;border:1px solid #e0e0e0;border-radius:14px;padding:14px 16px;margin:16px 0}.muted{color:#667781}</style></head><body><h1>Termos de Serviço</h1><small>Última atualização: ${today}</small><div class="box"><p class="muted">Estes Termos regem o uso do ZAPMRO (“Serviço”). Ao acessar ou usar o Serviço, você concorda com estes Termos e com a Política de Privacidade.</p><p class="muted">Política de Privacidade: <a href="${pp}">${pp}</a>.</p></div><h2>1. Elegibilidade</h2><p>Você declara ter capacidade legal para contratar e usar o Serviço em nome próprio ou da sua empresa.</p><h2>2. Conta e acesso</h2><ul><li>Você é responsável por manter suas credenciais seguras e por toda atividade na sua conta.</li><li>Você deve fornecer informações corretas e mantê-las atualizadas.</li></ul><h2>3. Uso permitido</h2><p>Você concorda em usar o Serviço apenas para fins lícitos e em conformidade com as políticas das plataformas integradas (ex.: WhatsApp/Meta, Google) e com a legislação aplicável.</p><h2>4. Conteúdo e mensagens</h2><ul><li>Você é o único responsável pelo conteúdo que envia/agenda, suas listas de contatos e suas automações.</li><li>Você deve obter consentimentos e permissões necessários para contatar pessoas e processar dados pessoais.</li><li>Você não deve enviar spam, conteúdo ilícito, fraudulento, ofensivo ou que viole direitos de terceiros.</li></ul><h2>5. Integrações (WhatsApp/Meta, Google e outras)</h2><ul><li>O funcionamento depende de serviços de terceiros. Interrupções, limitações e mudanças nesses serviços podem afetar o Serviço.</li><li>Você concorda em respeitar limites, políticas e requisitos técnicos dos provedores.</li></ul><h2>6. Disponibilidade e manutenção</h2><p>O Serviço pode passar por atualizações, manutenções e mudanças. Podemos modificar ou descontinuar recursos para manter segurança e conformidade.</p><h2>7. Suspensão e encerramento</h2><p>Podemos suspender ou encerrar o acesso se houver indícios de abuso, violação destes Termos, riscos de segurança ou exigência legal. Você pode solicitar encerramento/exclusão conforme a Política de Privacidade.</p><h2>8. Limitação de responsabilidade</h2><p>Na extensão máxima permitida em lei, o Serviço é fornecido “como está”. Não garantimos que o Serviço será ininterrupto ou livre de erros. Não nos responsabilizamos por perdas indiretas, lucros cessantes ou danos decorrentes do uso de integrações de terceiros.</p><h2>9. Privacidade</h2><p>O tratamento de dados é regido pela Política de Privacidade.</p><h2>10. Contato</h2><p>Suporte e contato: <a href="mailto:${escapeHtmlServer(CONTACT_EMAIL)}">${escapeHtmlServer(CONTACT_EMAIL)}</a> ou <a href="${home}">${home}</a>.</p><h2>11. Alterações dos Termos</h2><p>Podemos atualizar estes Termos. A data de “última atualização” indica a versão vigente.</p></body></html>`);
});
route('GET', '/termosdoservico/', routes.GET['/termosdoservico']);
route('GET', '/br/termosdoservico', routes.GET['/termosdoservico']);
route('GET', '/br/termosdoservico/', routes.GET['/termosdoservico']);

route('GET', '/exclusaodedados', (req, res) => {
  const today = new Date().toISOString().slice(0, 10);
  const home = 'https://zapmro.com.br';
  html(res, `<!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Exclusão de Dados - ZAPMRO</title><link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg%20xmlns=%27http://www.w3.org/2000/svg%27%20viewBox=%270%200%2064%2064%27%3E%3Ccircle%20cx=%2732%27%20cy=%2732%27%20r=%2730%27%20fill=%27%2325d366%27/%3E%3Cpath%20fill=%27%23ffffff%27%20d=%27M42.5%2036.8c-1.1-.6-2.5-1.3-3.7-1.8-.4-.2-.9-.1-1.2.2l-2%202.4c-.3.3-.7.4-1.1.3-2.7-1.1-5.9-4.2-7-7-.1-.4%200-.8.3-1.1l2.4-2c.3-.3.4-.8.2-1.2-.5-1.2-1.2-2.6-1.8-3.7-.2-.4-.6-.7-1.1-.7H23c-.6%200-1.1.4-1.2%201-1.1%206.3%201%2012.6%205.9%2017.5%204.9%204.9%2011.2%207%2017.5%205.9.6-.1%201-.6%201-1.2v-3.1c0-.5-.3-.9-.7-1.1z%27/%3E%3C/svg%3E"><style>body{font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;max-width:900px;margin:40px auto;padding:0 16px;line-height:1.55;color:#111}h1{font-size:28px;margin:0 0 8px}h2{font-size:18px;margin:26px 0 8px}p,li{color:#333}a{color:#128c7e}small{color:#666}.box{background:#fff;border:1px solid #e0e0e0;border-radius:14px;padding:14px 16px;margin:16px 0}.muted{color:#667781}</style></head><body><h1>Exclusão de Dados</h1><small>Última atualização: ${today}</small><div class="box"><p class="muted">Esta página descreve como solicitar a exclusão de dados relacionados ao uso do ZAPMRO e integrações (ex.: login com Facebook e conexão WhatsApp Business/Cloud API).</p></div><h2>1. Solicitação por e-mail</h2><p>Envie um e-mail para <a href="mailto:${escapeHtmlServer(CONTACT_EMAIL)}">${escapeHtmlServer(CONTACT_EMAIL)}</a> com o assunto <strong>“Exclusão de dados”</strong> e informe:</p><ul><li>Seu e-mail de acesso ao ZAPMRO</li><li>Seu nome</li><li>Se aplicável, o ID do Facebook (ou print do perfil) usado no login</li><li>O que deseja excluir: conta, histórico, integrações, ou “tudo”</li></ul><h2>2. Prazo</h2><p>Respondemos e processamos a solicitação em prazo razoável, podendo solicitar confirmação de identidade para sua segurança.</p><h2>3. O que é excluído</h2><ul><li>Conta e credenciais (quando solicitado)</li><li>Tokens de integrações (ex.: Google/Meta)</li><li>Configurações (ex.: funis, automações, agendamentos)</li><li>Históricos e registros relacionados ao painel, conforme aplicável e permitido por lei</li></ul><h2>4. Observações</h2><p>Alguns registros podem ser retidos quando necessário para cumprimento legal, prevenção a fraude ou segurança.</p><h2>5. Links úteis</h2><ul><li><a href="${home}/politicadeprivacidade">${home}/politicadeprivacidade</a></li><li><a href="${home}/termosdoservico">${home}/termosdoservico</a></li></ul></body></html>`);
});
route('GET', '/exclusaodedados/', routes.GET['/exclusaodedados']);
route('GET', '/br/exclusaodedados', routes.GET['/exclusaodedados']);
route('GET', '/br/exclusaodedados/', routes.GET['/exclusaodedados']);
